...
可选属性 `date` 设置文章日期（如 `2026-09-14`）。没有该属性时，已有笔记使用 Git 中首次公开的日期；新笔记使用首次导出到公开快照的时间。`Posted` 会保持不变；公开内容后续发生变化时，文章页会同时显示 `Updated`，更新时间不会因公开快照未变的重复构建而刷新。这两类时间和公开内容指纹均保存在 `docs/publication.json`。公开的目录 `index.md` 优先作为该目录首页，顶层栏目索引不重复加入首页文章列表。文章页标题下不自动重复正文摘要；只有笔记显式填写 `description` 时才显示简介。
