...
!!! tip "序列的选取"
    ...
    1.希尔增量序列：$h_t=\lfloor N/2\rfloor, h_k = \lfloor h_{k+1}/2\rfloor$
    最坏复杂度 $O(N^2)$（即只在 1-sort 时进行了排序）
    因为相邻增量不互质，所以奇偶的只会在最后1-sort中比较
    ...
    如何计算一个序列的复杂度：尚未解决，但是不可能达到纯粹的$O(NlogN)$
...
- 算法二：
    ...
    //Heap Sort position start from 0.
    ```
