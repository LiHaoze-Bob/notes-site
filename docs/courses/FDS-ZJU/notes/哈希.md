...
title: 哈希
---
!!! warning "这部分的 PPT 真的是乱的离谱，看的半懂不懂"
...
- 哈希函数应该是 unbiased 的，即 $P(f(x)=i)=\dfrac{1}{b}$，这种函数称为均匀哈希函数（uniform hash function）
- 对于整数的哈希，例如 $f(x) = x \bmod \text{tableSize}$，其中 tableSize 最好选择一个质数，这样对于随机输入，关键字的分布更均匀
...
- 即有多个哈希函数 $h_0(x), h_1(x), \cdots$，其中 $h_i(x) = (\text{hash}(x) + f(i)) \bmod \text{tableSize}$
...
- 即 $f(i)=i \cdot hash_2(x)$
- 一般选择 $hash_2(x)=R-(x \bmod R)$，其中 $R$ 为小于表大小的质数
