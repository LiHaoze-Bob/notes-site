...
!!! hint
    学习率决定每次更新的整体步长，优化器决定如何利用当前梯度以及历史梯度。通俗来讲，如果说学习率调度是调整步长，那么优化器调整就是调整下山的方法。



设第 $t$ 次更新时，参数为 $\theta_t$，当前 mini-batch 计算出的梯度为
...
!!! note
    用“容易陷入局部最优”概括 SGD 的困难。在高维神经网络中，实际还经常遇到鞍点、狭长谷底中的震荡，以及不同方向梯度尺度差异很大的问题。
...
!!! hint
    严格来说，标签平滑和焦点损失并不是正则化方法，它可以间接改善泛化能力，但是它们更像是在损失函数做功夫。


独热标签把正确类别设为 $1$，其余类别全部设为 $0$。

交叉熵会推动模型不断增大正确类别与其他类别之间的 logit 差距。若要求 softmax 概率严格达到 $1$ 和 $0$，理论上需要无限大的 logit 差距，因此模型容易形成过度自信的预测。

标签平滑将 hard label 改为 soft label。采用“把 $\varepsilon$ 分给其余 $K-1$ 类”的形式：
...
!!! tip
    简单来说，焦点损失就是训练时让模型更加关注没有学会的样本。“错题集”何尝不是一种Focal Loss(hhh
...
Feature Pyramid Network 在自底向上的 CNN 特征之外，增加自顶向下路径和横向连接，保留一组不同分辨率的特征图

FPN 常用于目标检测，尤其有助于检测小目标。

## 超参数优化 {#超参数优化}

超参数则需要在训练前或训练过程中人为设定，可以通过试验和调优确定
...
ResNet 使用逐元素相加，特征图通道数通常保持不变；DenseNet 使用通道拼接，保留并复用前面各层的特征


!!! success
    🥲终于写完了，由于后半部分涉及的论文太多了，可以拓展的也很多，所以后半部分由codex辅助写作完成，笔者review&rewrite。感兴趣的读者可以查阅有关论文进一步了解（👀
